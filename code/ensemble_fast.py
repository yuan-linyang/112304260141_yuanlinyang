...
def main():
    ...
    # 2. 预处理
    print("\n【2】预处理")
    train_words = [clean_review(r) for r in train_df['review'][:20000]]  # 加速
    test_words = [clean_review(r) for r in test_df['review']]
    unlabeled_words = [clean_review(r) for r in unlabeled_df['review'][:30000]]  # 加速
    all_words = train_words + unlabeled_words + test_words
    
    y_train = train_df['sentiment'].values[:20000]
    ...
    models = [
        ...
        lambda s: ExtraTreesClassifier(n_estimators=100, max_depth=12, random_state=s)
    ]
    ...
    submission_path = os.path.join(SUBMISSION_DIR, 'submission_ensemble_v2.csv')
    ...
    pd.DataFrame(list(stats.items()), columns=['指标', '数值']).to_csv(
        os.path.join(RESULTS_DIR, 'ensemble_v2_stats.csv'), index=False, encoding='utf-8-sig'
    )
